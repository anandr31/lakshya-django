...
from utils.models import LakshyaTestimonial, Partner
...
def get_donation_details_for_analytics(request):
    ...
    # list of tuples - name, batch, branch, amount, last Donated on
    donor_details_list = []
    for temp_dict in Donation.objects.values('donor').annotate(total=Sum('amount')):
        ...
        last_donated_on = Donation.objects.filter(donor=donor).order_by(
            "-date_of_donation")[0].date_of_donation
        donor_details = (donor.name, donor.year_of_passing,
                         donor.get_department_display, total, last_donated_on.strftime("%d-%m-%Y"))
...
def get_donor_emails_for_campaigns(request):
    emails = [user.email for user in User.objects.filter(
        person__donation__isnull=False).distinct() if user.email]
    return render_to_response("get-donor-emails-for-campaigns.html",
                              RequestContext(request, {"email_addresses": ", ".join(emails)}))


class PartnersView(TemplateView):
    template_name = "lakshya/partners.html"
