...
from innovation.models import IspApplication, NEED_TO_REVIEW, \
    InnovationInstrument


def get_innovation_list(request, project_name):
    if not project_name:
        project_name = "hovamarine"
    return render_to_response("innovations_list.html", RequestContext(request, {'project_name': project_name}))
    ...
    if request.method == 'POST':  # If the form has been submitted...
        ...
        if form.is_valid():  # All validation rules passes
            try:
                ...
                user = User.objects.create(username=form.cleaned_data['email'][:28],
            ...
            IspApplication.objects.create(date_of_submission=date.today(),
                                          year_of_submission=date.today().year, abstract=form.cleaned_data['abstract'],
                                          title=form.cleaned_data['title_of_project'], status=NEED_TO_REVIEW,
                                          member=person, other_member_details=form.cleaned_data['other_member_details'])
...
class InnovationHomeView(TemplateView):
    template_name = 'innovation/home.html'

    def get_context_data(self, **kwargs):
        context = super(InnovationHomeView, self).get_context_data(**kwargs)
